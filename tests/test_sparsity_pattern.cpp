...
int main()
{
    ...
    suite<"SparsityPattern"> sp_suite = [] {
        ...
        test("within each row, column indices are strictly increasing") = [] {
            mi::SparsityPattern<double> p(
                1, 4,
                {{.row = 0, .col = 3, .value = 1}, {.row = 0, .col = 0, .value = 2}, {.row = 0, .col = 2, .value = 3}});
        ...
        test("out-of-range row throws out_of_range") = [] {
            expect(throws<std::out_of_range>(
                [] { mi::SparsityPattern<double> p(2, 2, {{.row = 2, .col = 0, .value = 1}}); }));
        };

        test("out-of-range column throws out_of_range") = [] {
            expect(throws<std::out_of_range>(
                [] { mi::SparsityPattern<double> p(2, 2, {{.row = 0, .col = 2, .value = 1}}); }));
        };

        test("float instantiation") = [] {
            mi::SparsityPattern<float> p(2, 2,
                                         {{.row = 0, .col = 0, .value = 1.5F}, {.row = 1, .col = 1, .value = 2.5F}});
            expect(p.nonzeros() == 2_u);
